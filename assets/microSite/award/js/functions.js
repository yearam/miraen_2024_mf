...
(function($){
		...
		lazyLoad: function() {
			...
			if( lazyLoadEl.length > 0 ) {
				lazyLoadEl.each( function(){
					...
					element.attr( 'src', '/static/event/together100_new/images/blank.svg' ).css({ 'background': 'url(/static/event/together100_new/images/preloader.gif) no-repeat center center #FFF' });
	...
	SEMICOLON.header = {
		...
		menufunctions: function(){
			...
			$( '.top-links ul li:has(ul) > a, #primary-menu.with-arrows > ul > li:has(ul) > a > div, #primary-menu.with-arrows > div > ul > li:has(ul) > a > div, #page-menu nav ul li:has(ul) > a > div' ).append( '<i class="xi-angle-down"></i>' );
			...
			if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ) {
				$('#primary-menu.sub-title > ul > li').hover(function() {
					...
					$(this).prev().css({ backgroundImage : 'url("/static/event/together100_new/images/icons/menu-divider.png")' });
				});
			...
			if( SEMICOLON.isMobile.Windows() ) {
				...
				$( '#primary-menu ul li:has(ul)' ).append('<a href="#" class="wn-submenu-trigger"><i class="xi-angle-down"></i></a>');
			...
			if( $onePageMenuEl.length > 0 ){
				...
				$onePageMenuEl.find('a[data-href]').off( 'click' ).on( 'click', function(){
					var element = $(this),
						...
						divScrollMargin = element.attr('data-margin') || 0;
					...
					if( $( divScrollToAnchor ).length > 0 ) {
						...
						console.log('scrollTop',$( divScrollToAnchor ).offset().top,  Number(divScrollOffset));
						divScrollOffset = Number(divScrollOffset) + Number(divScrollMargin);
	...
	SEMICOLON.widget = {
				...
				$flexSliderEl.each(function() {
					...
					$flexsSlider.flexslider({
						...
						start: function(slider){
							...
							SEMICOLON.initialize.lightbox();
							$('.flex-prev').html('<i class="xi-angle-left"></i>');
							$('.flex-next').html('<i class="xi-angle-right"></i>');
		...
		navTree: function(){
			...
			if( $navTreeEl.length > 0 ){
				$navTreeEl.each( function(){
					...
					element.find( 'ul li:has(ul) > a' ).append( ' <i class="xi-angle-down"></i>' );
		...
		notifications: function( element ){
			...
			toastr.options.closeHtml = '<button><i class="xi-close"></i></button>';
				...
				element.find('form').validate({
					submitHandler: function(form) {
						...
						if( elementLoader == 'button' ) {
							...
							defButton.html('<i class="xi-spinner-1 icon-spin nomargin"></i>');
						} else {
				...
				element.find('form').validate({
					submitHandler: function(form) {
						...
						if( elementLoader == 'button' ) {
							...
							$(form).find('.icon-email2').removeClass('icon-email2').addClass('xi-spinner-1 icon-spin');
						}
						...
						$(form).ajaxSubmit({
							...
							success: function( data ) {
								if( elementLoader == 'button' ) {
									...
									$(form).find('.xi-spinner-1').removeClass('xi-spinner-1 icon-spin').addClass('icon-email2');
								}
								if( data.alert != 'error' && elementRedirect ){
									window.location.replace( elementRedirect );
									return true;
