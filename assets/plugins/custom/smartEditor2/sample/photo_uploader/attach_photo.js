    ...
    function drop(ev) {
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			//초기화
			console.log("이브이",ev.dataTransfer.files);
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					...
					if ( j < nMaxImageCount ){
						...
						var url = '/pages/api/editerupload.ax'; // 파일을 업로드할 서버의 엔드포인트 URL
						var formData = new FormData();
						formData.append('data', new Blob([JSON.stringify(data)], { type: "application/json" }));
						formData.append('files', files[i]);
						formData.append('directoryCode', 'directoryCode');
    ...
    function html5Upload() {
    	...
    	sUploadURL= '/pages/api/editUpload.ax'; 	//upload URL
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
					console.log("파일1",tempFile);
					console.log("파일 id - "+j,tempFile.id);
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		var formData = new FormData();
		formData.append('file', tempFile.id);
		console.log(tempFile.id)
    	var oAjax = jindo.$Ajax(sUploadURL, {
			type: 'xhr',
			method : "post",
			data: formData,
			processData: false, // FormData 처리를 jQuery에 위임하지 않음
			contentType: false,
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						console.log("파일2",res._response.responseText);
		...
		oAjax.header("file-id",encodeURIComponent(tempFile.id));
		oAjax.header("file-size",tempFile.size);
		oAjax.header("file-Type",tempFile.type);
		oAjax.request(tempFile.id);
    ...
    function makeArrayFromString(sResString){
		...
		console.log("파일3",aResult[aResultleng])
