...
nhn.husky.SE_PasteHandler = jindo.$Class({
	...
	_handlePaste : function(we){
		...
		// --[SMARTEDITORSUS-2005]
			...
			// 브라우저의 고유 붙여넣기 동작으로 콘텐츠가 본문 영역에 붙여넣어진다.
			setTimeout(jindo.$Fn(function(){
				// [SMARTEDITORSUS-1676]
				/**
				 * 콘텐츠가 붙여넣어지는 과정에서
	...
	/**
	 * 붙여넣어지는 외부 프로그램의 콘텐츠를 조작하기 위한 준비를 한다.
	 * */
	...
	/**
	 * 외부 프로그램의 콘텐츠가 원문에 붙여넣어지는 과정을 진행한다.
	 * */
	_processPaste : function(){
		...
		if(this._sTarget){
			...
			try{
				...
				// 붙여넣기 작업 공간에 붙여넣은 콘텐츠를 떠서 본문의 해당 영역 교체
				this._loadToBody();
	...
	/**
	 * 본문 영역에 외부 프로그램의 콘텐츠가 붙여넣어지면 이를 저장하고,
	...
	_filterPastedContents : function(sOriginalContent, bUseTableFilter){
		// 문단 교체와 관련된 변수
		this._isPastedMultiParagraph = false; // 붙여넣어지는 콘텐츠가 여러 문단으로 구성되어 있는지 확인
		...
		aResult = [], // 최종적으로 반환되는 정제된 콘텐츠들이 담긴 배열
			...
			while(sOriginalContent != ""){
				...
				if(rxOpeningAndClosingTag.test(sOriginalContent)){ // <tagName />
					...
					if(rxMultiParagraphIndicator.test(sTagName)){
						this._isPastedMultiParagraph = true; // 붙여넣어진 콘텐츠가 여러 문단으로 구성되어 있는지 확인
	 ...
	 * 본문 영역에 붙여넣을 콘텐츠를 선별하여 
	 * 브라우저 고유의 붙여넣기 기능으로 본문 영역에 붙여넣은 기존 콘텐츠와 교체한다.
	 * */
	_loadToBody : function(){
		...
		try{
			 ...
			 * As-Is 콘텐츠
			 * 
			 * 본문 영역에 붙여넣어진 콘텐츠 중 가공된 콘텐츠로 치환될 대상 목록을 획득
			 * */
			oSelection.moveToStringBookmark(this._sBM);
			oSelection.select();
			 ...
			 * oSelectionClone을 기반으로 브라우저 고유 기능으로 붙여넣었던 콘텐츠를 복원한다.
			 * */
			// 삽입된 콘텐츠 제거
			oSelection.moveToStringBookmark(this._sBM);
			oSelection.select();
