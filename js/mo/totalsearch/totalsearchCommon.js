...
class Totalsearch {
  constructor() {
    this.appendCategory = function (tabId, $el, $btn, $filter) {
      $(`${tabId} .filter-wrap .buttons`).empty();
      $(`${tabId} .filter-wrap .filter-items`).remove();

      $(`${tabId} .filter-wrap .buttons`).append($btn);
      $(`${tabId} .filter-wrap .filters`).after($el);

      $('.popup-sheet ul.filter-list').empty();
      $('.popup-sheet ul.filter-list').append($filter);

      filterFunc();
      toggleThis('.filter-items button');
      renderSwiper();
      $(`${tabId} .filter-wrap`).removeClass('display-hide');
    };
        ...
        const options = {
          ...
          success: function (res) {
            ...
            if (!_.isNil(callback) && typeof callback === 'function') {
              callback(res);
              $('.detailed-search .search-inner > h2 > strong').text(res.row.totalCount.toLocaleString());
            }
          }
        };
  ...
  screen = {
    ...
    f: {
      ...
      executeSearch: () => {
        ...
        keyword && screen.f.addKeyword(keyword);
      },
      ...
      handleMainKeyword: (urlKeyword) => {
        ...
        const inputValue = $('section[data-type="totalsearch-section"] .search-box > input').val().trim();
        ...
        // 키워드 리스트에 검색어 띄우기
        let keywordList = $('.detailed-search .search-inner');
        keywordList.html(`<h2> ‘${screen.v.mainKeyword}’ 의 검색결과는<br /> 총 <strong class="text-primary">0</strong>건 입니다.</h2>`);
      ...
      getSearchText: () => {
        ...
        let search = '';
        if (!!search) {
         search = searchTxtValue.replace(',', ' '); // searchTxtValue 값이 없는 경우 에러 발생
        }
    ...
    event: function () {
      ...
      $('section[data-type="totalsearch-section"] .search-box > button').on('click', () => {
        screen.f.handleMainKeyword();
      })

      $('section[data-type="totalsearch-section"] .search-box > input').on('keypress', (e) => {
        if (e.which === 13) {
          screen.f.handleMainKeyword();
          $('section[data-type="totalsearch-section"] .search-box > input').val('');
        }
      })
