...
$(function() {
    textbookDetailScreen = {
            ...
            getMasterInfo: async () => {
                ...
                const options = {
                    ...
                    success: async function (res) {
                        ...
                        // 교과서 정보 영역 초기화
                        $('#main-contents .textbook-top-area').remove();
                        ...
                        if (masterInfo.allSmartPptUseYn === "Y" && masterInfo.allSmartPptDisplayYn === "Y") {
                            smartPptElement = `
                             <button data-type="smartPptDownloadBtn" data-btn-type="page-open" data-path="${masterInfo.allSmartPptFilePath ? masterInfo.allSmartPptFilePath.replace(/ /g, "%20") : ''}" type="button" class="button size-md type-primary fluid"> 스마트수업 
                            </button>`
                        }
                        ...
                        textbookDetailScreen.v.textbookDetailHtml = `
                           <section class="full">
                              <div class="textbook-wrap">
                                <a href="javascript:void(0);" class="textbook book-link-mobile" data-path="${ebookFilePath?.replace(/ /g, "%20")}" data-btn-type="page-open">
                                ...
                                <div class="inner-wrap">
                                  <div class="unit">
                                    ...
                                    <div class="buttons">
                                      ${ebookCopyElement} 
                                      <button id="reg-favorite" type="button" class="bookmark">즐겨찾기</button>
                                    </div>
                                  </div>
                                  <h2>${masterInfo.textbookName} ${(masterInfo.leadAuthorName !== null && masterInfo.leadAuthorName !== undefined) ? `(${masterInfo.leadAuthorName})` : ''}</h2>
                                  ${smartPptElement}
                                </div>      
                          </section>
                          <section class="type-thick special-data">
                                  <div class="data-items">
                                  ${specialDataList.length !== 0 ?
                            specialDataList.map(item =>
                                `<a href="javascript:void(0);" data-type="specialData" data-extension="${item.fileExtension}" data-save-code=${item.fileUploadMethod} data-path=${item.filePath?.replace(/ /g, "%20")}>
                                            <span>${item.referenceName}</span>
                                            <i class="icon">
                                                <svg>
                                                    <use xlink:href="/assets/images/svg-sprite-solid.svg#icon-${item.fileUploadMethod.toLowerCase() === 'link' || item.fileUploadMethod.toLowerCase() === 'newwin'? 'new-windows-single' : 'download'}"></use>
                                                </svg>
                                            </i>
                                        </a>`
                            ).join('') : ''}
                        ...
                        // 교과서 특화자료 클릭 이벤트
                        $('[data-type="specialData"]').on('click', function () {
                        ...
                        // 이북, 스마트수업 클릭 이벤트
                        $('.book-link-mobile, button[data-btn-type="page-open"]').on('click', function () {
                            if ($isLogin) {
                                if ($('#userGrade').val() === '001') {
                                    textbookDetailScreen.f.needAuthAlert();
                                } else if ($('#userGrade').val() === '002') {
                        ...
                        // 열람 이력 저장
                        let depth1 = res.row.subjectName;
                        let depth2 = res.row.subjectRevisionCode;
                        let depth3 = res.row.textbookName;
                        if (res.row.leadAuthorName != null && res.row.leadAuthorName != '') depth3 = res.row.textbookName + ' (' + res.row.leadAuthorName + ')';

                        $cmm.setHistoryCookie(7, 'history-mid', `${depth1}:${depth2}:${depth3}:${location.pathname + location.search}`, '/pages/mid');

                        // header에 GNB 제거 -> 교과서 이름(+ 저자명), 뒤로가기 버튼 추가 // TODO - 24.04.08 기획서, 퍼블 화면 비교 후 수정 예정
                        let textbookNameAndAuthor = masterInfo.textbookName;

                        if (masterInfo.leadAuthorName !== null && masterInfo.leadAuthorName !== '' && masterInfo.leadAuthorName !== undefined) {
                            textbookNameAndAuthor = masterInfo.textbookName + ' (' + masterInfo.leadAuthorName + ')';
                        }
                        ...
                        // $('header > section.header-bar, header > section.gnb').remove(); // 잔상 방지를 위해 header에서 display-hide
                        $('header').append(headerTextbookHtml);

                        // 팝업 > 교과서명 영역 text
                        $('#popup-textbook p.ellipsis-multi').html(textbookNameAndAuthor);
                    }
                };
            ...
            getUnitListTo2Depth: async (data) => {
                ...
                try {
                    ...
                    if (unitList.length !== 0) {
                        ...
                        // 교과서 정보 하단에 현재 단원명 표시 - - 모바일 X
                        //textbookDetailScreen.f.setUnitName('unit');

                        const unitListContainer = $('#popup-textbook .popup-container .list-items');
                            ...
                            ${result.map(function (item, index) {
                                return `
                                        <a href="#" data-depth="1" data-unitseq="${item[0].unitSeq}">
                                            ...
                                            <span data-depth="1" data-unitName="${item[0].unitName}">${item[0].unitName}</span>
                                        </a>
                                        `
                        }).join('')}
                        `);
                    ...
                    // $('#popup-textbook .popup-container .list-items a:first').addClass('active');
                    //
                    // $('.type-thick .type-textbook').html(
                    //     $('#popup-textbook .popup-container .list-items a:first').text()
                    // );
                    //
                    // $('#popup-textbook .ellipsis-multi').html(
                    //     $('#popup-textbook .popup-container .list-items a:first').text()
                    // );
        ...
        init: async function (event) {
            ...
            // sitemap -> 교과서 상세 이동 시 해당 gnb에 .active 추가
            let subjectCode = ''; // 교과서 코드
            let activeGnbTarget = ''; // .active 추가 할 gnb
            if (searchParams.get('subjectCode') !== null && searchParams.get('subjectCode') !== undefined) {
                subjectCode = searchParams.get('subjectCode');
                $('#gnbAreaMid > a').each(function() {
                    // subjectCode에 '/' 또는 '|' 구분자가 없을 때
                    if ($(this).attr('id') !== null && $(this).attr('id') !== undefined
                        && !(subjectCode.includes('/') || subjectCode.includes('|'))
                        && $(this).attr('id').includes(subjectCode)) {
                        activeGnbTarget = $(this);
                        // subjectCode에 '/' 또는 '|' 구분자가 있을 때
                    } else if ($(this).attr('id') !== null && $(this).attr('id') !== undefined
                        && (subjectCode.includes('/') || subjectCode.includes('|'))
                        && $(this).attr('id').includes('|')
                        && $(this).attr('id').split('|').join('/').includes(subjectCode)) {
                        activeGnbTarget = $(this);
                    }
                });
                $('#gnbAreaMid > a').addClass('active');
                $('#gnbAreaMid > a').removeClass('active');
                activeGnbTarget.addClass('active');
            }

            $('#main-contents section.textbook-top-area').empty();
