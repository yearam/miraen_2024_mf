$(function () {
  let screen = {
    ...
    c: {
          ...
          success: function(result) {
            if (result.status == 'succ') {
              ...
              if (list.length == 0) {
                ...
                html += '<fieldset>';
                html += '<div class="clear-container size-xl">';
                html += '<input type="text" class="size-xl" title="학교명 입력란" name="" id="school-name" placeholder="학교명을 입력해 주세요." />';
                html += '<button type="button" class="button-clear"><svg><title>삭제</title><use href="/assets/images/svg-sprite-solid.svg#icon-close-round" /></svg></button>';
                html += '</div>';
                ...
                html += '<input type="hidden" title="우편번호" name="" id="member_post-1" disabled/>';
                html += '<div class="clear-container  size-xl">';
                html += '<input type="text" class="size-xl" title="학교 주소 입력란" name="" id="address1-1" placeholder="주소를 검색해 주세요." disabled/>';
                html += '<button type="button" class="button-clear"><svg><title>삭제</title><use href="/assets/images/svg-sprite-solid.svg#icon-close-round" /></svg></button>';
                html += '</div>';
                html += '</fieldset>';
                html += '<fieldset>';
                html += '<div class="clear-container size-xl">';
                html += '<input type="text" class="size-xl" title="상세 주소 입력란" name="" id="address2-1" placeholder="상세 주소를 입력해 주세요." />';
                html += '<button type="button" class="button-clear"><svg><title>삭제</title><use href="/assets/images/svg-sprite-solid.svg#icon-close-round" /></svg></button>';
                html += '</div>';
                html += '</fieldset>';
      ...
      signUp: ()=> {
        ...
        for (let i=0; i<screen.v.validaionArr.length; i++) {
          ...
          if (val == "") {
            console.log(screen.v.validaionArr[i]);
        ...
        if (confirmFlag) {
          ...
          //생년월일 자리수 체크
          /*if ($("#step3 input[id='yy']").val().length != 4) {
            let todayDate = new Date();
            let startYear = todayDate.getFullYear();
            startYear = startYear -13;
            if($("#step3 input[id='yy']").val() > startYear || $("#step3 input[id='yy']").val() < 1950){
              screen.f.setErrorMsg('yy', 200, 400, '생년월일을 정확히 입력해주세요.', 'N');
              confirmFlag = false;
              return;
            }
          }*/
        ...
        if (screen.v.selectSchoolYn) {
          ...
          if (screen.v.teacherLevel == 'member') {
            ...
            if (screen.v.schoolLevel == 'ELEMENT') {
              if (!t05 && !t04 && !u04) {
                alert('학교정보 담임/과목/직위 값을 선택 해 주세요.')
                confirmFlag = false;
                return;
              }
            }
          ...
          success:function(result){
            ...
            if (result.status == 'succ') {
              ...
              if (sns == 'kakao') {
                ...
                alert('Faild Your Join Type. ['+$('#joinMethodCd').val()+ ']');
              }
            } else {
              alert(result.dispMessage)
            }
      ...
      callMailiCert: (e)=> {
        if (screen.v.smsCertProgress) {
          alert('진행 중인 인증이 있습니다. 인증 완료 혹은 시간만료 후에 다른 인증이 가능 합니다.');
          return;
        }
    ...
    f: {
      ...
      saveCertFile: (e)=> {
        ...
        if($.inArray(fileType, ['hwp' , 'pdf', 'jpg']) == -1) {
          alert(fileType+' 파일은 업로드 하실 수 없습니다. 첨부 가능한 확장자를 확인해 주세요.');
      ...
      selectSchool: (e)=> {
        ...
        if (screen.v.schoolRequest == 'Y') { // 검색결과 없음
          ...
          // validation
          if ($('#school-name').val() == '') {
            alert('학교 정보를 입력해 주세요.')
            validation = false;
            return false;
          }
          if ($('#address1-1').val() == '') {
            alert('학교, 학교 주소 정보를 입력해 주세요.')
            validation = false;
            return false;
          }
          ...
          alert('학교 등록 요청이 완료되었습니다. 학교 등록이 완료될 때까지 2~3일 정도 소요됩니다. ');
        } else { // 검색결과 있음
          // validation
          let activeCnt = 0;
          $('.school-select').each(function () {
      ...
      addTextbook: (e)=> {
        ...
        if (screen.v.textbookArr.includes(textbookSeq)) {
          ...
          let classNm = '';
          if (screen.v.schoolType == 'ETC') {
            classNm = '.'+reqClassNm+' .school-grade-box.school-etc .' + levelClassNm + ' .buttons.margin-top-lg';
          } else {
            classNm = '.'+reqClassNm+' .school-grade-box.' + levelClassNm + ' .buttons.margin-top-lg';
          }
          $(classNm).append(html);
      ...
      setErrorMsg: (id, scroll1, scroll2, msg, alertYn)=> {
        ...
        if(id == 'yy'){
          $('#'+id).parent().parent().children('#yy_span').text('* '+msg)
        }else{
          $('#'+id).parent().children('span').text('* '+msg)
    ...
    event: function () {
      ...
      // 엔터시 인증서 모듈 실행 방지
      $(document).on('keypress', '#step3 .forms input', (e)=> {
