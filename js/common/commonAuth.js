$(function () {
  let screen = {
          ...
          success: function(result) {
            if (result.status == 'succ') {
              ...
              if (list.length == 0) {
                ...
                html += '<input type="text" title="학교명 입력란" name="" id="school-name" placeholder="학교명을 입력해 주세요." />';
                ...
                html += '<div class="clear-container  size-xl">';
                html += '<input type="text" title="학교 주소 입력란" name="" id="address1-1" placeholder="주소를 검색해 주세요." disabled/>';
                html += '<button type="button" class="button-clear"><svg><title>삭제</title><use href="/assets/images/svg-sprite-solid.svg#icon-close-round" /></svg></button>';
                html += '</div>';
                html += '</fieldset>';
                html += '<fieldset>';
                html += '<div class="clear-container size-xl">';
                html += '<input type="text" title="상세 주소 입력란" name="" id="address2-1" placeholder="상세 주소를 입력해 주세요." />';
                html += '<button type="button" class="button-clear"><svg><title>삭제</title><use href="/assets/images/svg-sprite-solid.svg#icon-close-round" /></svg></button>';
                html += '</div>';
                html += '</fieldset>';
    ...
    event: function () {
      ...
      $(document).on('keypress', '#popup-authentication-code .cert-area input', (e)=> {
        if (e.keyCode == 13) {
          e.preventDefault();
          screen.c.checkCodeCert(e);
        }
      });
      // 서류 첨부 시 저장 - 교사/예비교사
