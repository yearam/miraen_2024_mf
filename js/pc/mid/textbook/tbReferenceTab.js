let tbReferenceTab;
const refDivCd5 = [429,2559,2565]; // 활용유형:교과서공통자료, 교과서 특화자료, 중간기말평가
$(function(){
    let resetV = {
    ...
    tbReferenceTab= {
        ...
        c: {
            ...
            commonItemList: (checkedItems) => {
                $.ajax({
                    type: "GET",
                    url: `/pages/mid/textbook/reference/commonList.ax?masterSeq=${tbReferenceTab.v.masterSeq}&refDivCd5=${checkedItems}`,
                    ...
                    success: function (res) {
                        ...
                        if (res.rows.length > 0) {
                            ...
                            $.each(res.rows, function (index, item) {
                                displayList = $(`
                                    <div id="commonItems-${index}" class="item commonItems">
                                        <span class="ox">
                                            <input 
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (categoryRows.length !== 0) {
                            ...
                            $(`.categoryCheckbox[data-category-contain=${selectedCategory}] div.filters .ox-group`).removeAttr('style');
                            ...
                            handleCheckbox('#ox-filter1-0', '#categoryTabContain .filter-list-checkbox');
                        } else {
                            // 표시될 체크박스가 없는 경우
            ...
            $('.classRefList, #commonItemList').on('click','button.ai-share',function (){
                ...
                if(checkTeacher()){
                    if(ai_class_popup_yn){
                        window.open(`${ai_site}/#/teacher/ai-class-share?worksheetId=${seq}&schoolType=mid`,"_blank",options);
                    }else{
            ...
            $('#categoryTabMid ul').on('click','li', async function () {
                ...
                if (tbReferenceTab.v.lastClickedRefTab !== selectedCategory) {
                    ...
                    if (selectedCategory === "005") {
                        ...
                        // 초기화 - 카테고리 탭 자료 영역
                        $(".classRefList").find('div.item').remove(); // 멀티미디어 자료
