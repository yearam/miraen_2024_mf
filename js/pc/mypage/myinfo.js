$(function () {
  let myScreen = {
    ...
    c: {
      ...
      myinfoModify: ()=> {
        ...
        if ($('#certHpYn').val() == 'N' && $('#certType').val() != '') {
          ...
          $alert.alert('인증 시에는 휴대폰 인증 필수 입니다.')
          confirmFlag = false;
          return;
        }
        ...
        if (scLevel == 'ELEMENT' && schoolSeq != '' && schoolSeq != null && schoolSeq != 0) {
          ...
          if (t05) {
            ...
            if ($('.school-grade-box.school-ele select').eq(0).val() == '') {
              $alert.alert('담임 값을 선택 해 주세요.')
              confirmFlag = false;
              return;
            }
          } else if (t04) {
            $('#step5 [name=dedicatedTeacherSubject]').val($('[name$="T04ele"]')[cnt].value);
            if ($('.school-grade-box.school-ele select').eq(1).val() == '') {
              $alert.alert('전담과목 값을 선택 해 주세요.')
              confirmFlag = false;
              return;
            }
          } else if (u04) {
            $('#step5 [name=etcTeacherType]').val($('[name^="U04"]')[cnt].value);
            if ($('.school-grade-box.school-ele select').eq(2).val() == '') {
              $alert.alert('직위 값을 선택 해 주세요.')
              confirmFlag = false;
              return;
            }
          } else {
      ...
      oneIdGateUrl : function(e){
        let siteUrl  = location.href;
        let goUrl = $(e.target).data("url");
        // location.href = "/sso/oneIdGate.mrn?goUrl=" + goUrl + "&siteUrl=" + siteUrl;
        window.open("/pages/common/sso/oneIdGate.mrn?goUrl=" + goUrl + "&siteUrl=" + siteUrl, "_blank");
      }
    },
    ...
    f: {
      ...
      setEleSelebox: (e)=> {
        let pEle = $(e.target).parents('.grid-wrap');
      ...
      addTextbook: (e) => {
        ...
        if (myScreen.v.textbookArr.includes(textbookSeq)) {
          ...
          $('.buttons.textbook-add-area').append(html);
          myScreen.v.textbookArr.push(textbookSeq)

          if (!$('.buttons.textbook-add-area').hasClass('margin-top-md')) {
            $('.buttons.textbook-add-area').addClass('margin-top-md');
          }
        }
      },
      ...
      delTextbook: (e) => {
        ...
        if (myScreen.v.textbookArr.length == 0) {
          $('.buttons.textbook-add-area').removeClass('margin-top-md');
        }
      },
        ...
        if (list.length > 0 || list != null) {
          ...
          for (let i=0; i<list.length; i++) {
            ...
            if (status == '1') {
              ...
              html += '<span class="badge size-xl type-round-box fill-green">+ '+item.point+'</span>'
            } else if (status == '2') {
              html += '<span class="badge size-sm type-round-box fill-primary">사용</span>'
              html += '<span class="date">'+item.date+'</span>'
              html += '<strong class="title">'+item.myPointTitle+'</strong>'
              html += '<div class="point">'
              html += '<span class="badge size-xl type-round-box fill-gray-500">- '+item.point+'</span>'
            } else if (status == '3') {
    ...
    event: () => {
      ...
      // 학교 선택시 교과서 리스트 초기화
      $(document).on('click', '#selectSchool', ()=> {
        myScreen.v.textbookArr = []
      });
      ...
      // 회원탈퇴
      $(document).on('click', '#cancelMembership', myScreen.c.oneIdGateUrl);
