...
var DEFAULT_ANIMATION_DURATION = 100;//700;
(function( $ ){
  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
	   ...
	   /**
		  ...
		  */
	    zoom : function(scaleToAdd, fixed, globalRequestedPosition, duration){
			...
			if(fixed) {
				// console.log("old", smartData.adjustedPosInfos.scale);
				newScale = (100*targetRect.width / originalSize.width) + 100*scaleToAdd;
				// console.log("new1", newScale);
				newScale = (newScale/10).toFixed(0);
				// console.log("new1-", newScale);
				newScale = Math.floor(newScale).toFixed(0) / 10;
				// console.log("new2", newScale);
			}
		...
		bind_mouse_event: function() {
			var smartData = targetElement.data('smartZoomData');
			if(!smartData) {
				return;
			}
			var containerDiv = smartData.containerDiv;
			var settings = smartData.settings;
			// listening mouse and touch events
			if(settings.touchEnabled == true)
				targetElement.bind('touchstart.smartZoom', touchStartHandler);
			if(settings.mouseEnabled == true){
    ...
    function stopAnim(userActionType){
    	...
    	if(smartData.transitionObject){ // if css transformation is surpported
			...
			var eventTypeToDispatch = "smart-zoom-event-stop";
			window.dispatchEvent(new CustomEvent(eventTypeToDispatch, {
				detail: {
					"scale": targetRect.width/originalSize.width,
					"pos": {x: targetRect.x, y:targetRect.y},
					"css": cssObject
				}
			}));
    	}else{
    ...
    function animate(target, globalLeft, globalTop, width, height, duration, callback){
    	...
    	if (smartData.transitionObject != null) { // use css transition if supported
			...
			var eventTypeToDispatch = "smart-zoom-event";
			window.dispatchEvent(new CustomEvent(eventTypeToDispatch, {
				detail: {
