    ...
    f: {
      ...
      renderResults: async (res) => {
        ...
        if (screen.v.param.siteType === 'ele') {
          for (const subTab in count) {
            ...
            $(tabId + ' a[data-collection="' + subTab + '"]').text(`${tabName}(${count[subTab].toLocaleString()})`);
          }
        } else {
          for (const subTab in count) {
        ...
        if (!tsAllScreen.v.menuListLength && !totalCount) {
          $('div[data-page="totalsearch"].box-no-data').css('display', 'block');
          $('div[data-page="totalsearch"].box-gray').css('display', 'block');
          ...
          $('div[data-page="totalsearch"].box-no-data span').text(keywords);
        } else {
          $('div[data-page="totalsearch"].box-no-data').css('display', 'none');
          $('div[data-page="totalsearch"].box-gray').css('display', 'none');
        }
            ...
            <div class="item size-textbook">
              <div class="inline-wrap gutter-md">
                ...
                <div class="text-wrap">
                  ...
                  ${document.subjectName ? `<p class="desc text-md">${document.subjectName}</p>` : ''}
                  ${document.unitName1 ? `
                    <div class="inline-wrap">
              ...
              <div class="buttons">
                ${activeTabType === 'ele' ? `
                  <button 
                    type="button" 
                    class="button size-lg type-secondary" 
          ...
          <div class="item ">
            ...
            <div class="buttons" data-item-index="${key}" data-tab-name="tsText">
              ...
              <button 
                ...
                data-type="download"
                type="button"
                ${tsAllScreen.f.ynToBooleanDisabled(TB.reference_download_use_yn)}
                >
                <svg>
                  <title>아이콘 다운로드</title>
                  <use xlink:href="/assets/images/svg-sprite-solid.svg#icon-download"></use>
      ...
      addButtonEvents: (parentNode,itemList,tabName) => {
        ...
        $(`${parentNode} [data-tab-name=${tabName}] [data-type=download]`).off('click').on('click',function () {
          const itemIndex = $(this).parent().data('itemIndex');
          const {originFileName,path,referenceFileId} = itemList[itemIndex].document;
          const teacher = tsAllScreen.f.checkTeacher('');
      ...
      getCheckedInputs : (parentId)=>{
        const checkedFilter = $(`#${parentId} input:checked`);
      ...
      handleFilter : function (e){
        ...
        const checkedInputList = tsAllScreen.f.getCheckedInputs(filterId);
        ...
        if(checkedInputList.length === filters.length - 1){
